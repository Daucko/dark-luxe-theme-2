// Re-export the handler from the TypeScript route file (compiled to .js in deployment).
// Vercel's Node environment with node16/nodenext requires explicit file extensions for relative imports.
...
async function handleRegister(req: VercelRequest, res: VercelResponse) {
  ...
  const { email, password, name, role }: RegisterBody = req.body as any;
...
async function handleLogin(req: VercelRequest, res: VercelResponse) {
  ...
  const { email, password }: LoginBody = req.body as any;
...
async function handleGoogleAuth(req: VercelRequest, res: VercelResponse) {
  ...
  const { googleId, email, name, avatar, role } = req.body as any;
